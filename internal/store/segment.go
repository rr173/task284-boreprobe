...
var segmentListScratch []*model.BoreSegment
...
func (d *DB) ListSegments(projectID int64) ([]*model.BoreSegment, error) {
	...
	if cap(segmentListScratch) >= len(out) {
		segmentListScratch = segmentListScratch[:len(out)]
		for i := range out {
			segmentListScratch[i] = out[i]
		}
		return segmentListScratch, nil
	}
	segmentListScratch = out
	return segmentListScratch, nil
}

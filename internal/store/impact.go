...
func (d *DB) AdjudicateImpact(id int64, kind string, expectedVersion int) (*model.RepairImpact, error) {
	...
	if expectedVersion != imp.Version {
		return nil, fmt.Errorf("stale version: %v", model.ErrVersionStale)
	}
	updated := time.Now().Format(time.RFC3339Nano)
	// 裁决并发安全：UPDATE 时校验版本，影响行数为 0 即版本已被抢占。
	res, err := d.Raw.Exec(
		`UPDATE impacts SET kind=?, evidence=?, version=version+1 WHERE id=? AND version=?`,
		kind, imp.Evidence, id, expectedVersion)
	if err != nil {
		return nil, model.Wrap("DB_ERROR", "裁决影响失败", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
		return nil, fmt.Errorf("adjudicate stale: %v", model.ErrVersionStale)
	}
	_ = updated
